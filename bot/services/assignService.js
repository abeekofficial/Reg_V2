...
async function assignOrder(bot, orderId) {
  try {
    ...
    if (order.status !== "pending") {
      logger.warn("assignOrder: order pending emas", {
        orderId,
        status: order.status,
      });
      return;
    ...
    const drivers = await getAvailableDrivers(
      order.from,
      order.to,
      config.order.maxDriversPerOrder,
    );
...
async function handleDriverAccept(
  ...
  if (busy) {
    await bot.answerCallbackQuery(query.id, {
      text: "❌ Sizda yakunlanmagan buyurtma bor!",
      show_alert: true,
    });
    return resolve(false);
  }
  ...
  // Driverga: passenger ma'lumotlari + safar boshlash tugmasi
  try {
    const pInfo =
      `\n\n👤 <b>${passenger.name}</b>\n` +
      `📱 <b>${passenger.phone}</b>\n` +
          ...
          inline_keyboard: [
            [
              ...
              },
              {
                ...
                callback_data: `cancel_trip_${order._id}`,
              },
            ],
          ],
        },
...
async function sendOrderToGroups(bot, order) {
  try {
    ...
    // Guruhlarni orderType bo'yicha filterlash
    // "all" → ikkalasi ham, "passenger" → faqat yo'lovchi, "cargo" → faqat yuk
    const groups = await Group.find({
      isActive: true,
      $or: [
        { orderTypes: "all" },
        { orderTypes: order.orderType },
        { orderTypes: { $size: 0 } }, // bo'sh array = eski guruhlar (eski ma'lumotlar)
      ],
    }).lean();
    const passenger = await User.findOne({
      telegramId: order.passengerId,
    ...
    if (!passenger) {
      logger.error("sendOrderToGroups: passenger topilmadi");
      return;
    }
    if (groups.length === 0) {
      logger.warn("Faol guruh topilmadi");
      return;
    }
